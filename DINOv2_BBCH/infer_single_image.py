...
def main():
    ...
    parser.add_argument("--station-code", default=None, help="Station code such as 02.02. Used for weather and location/elevation metadata.")
    ...
    image_backbone = checkpoint.get("image_backbone", DINO_DEFAULT_BACKBONE)
    ...
    if location_feature_dim > 0:
        if not args.station_code:
            raise ValueError("Checkpoint expects location metadata; provide --station-code.")
        location_features = station_location_features(args.station_code, strict=True).unsqueeze(0).to(device)
