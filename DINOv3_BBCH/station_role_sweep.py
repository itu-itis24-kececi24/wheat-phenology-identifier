...
def build_precompute_command(args, cache_path: Path) -> List[str]:
    return [
        ...
        "--dense-grid-size",
        str(args.dense_grid_size),
        "--image-backbone",
        args.image_backbone,
        "--device",
        args.device,
    ] + (["--pretrained"] if args.pretrained else []) + (
        ["--dense-features"] if args.dense_features else ["--no-dense-features"]
    ) + (["--dense-include-cls"] if args.dense_include_cls else ["--no-dense-include-cls"])
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Precompute DINOv3 dense embeddings once, train every 12-train/3-val/no-test station combination, "
    ...
    parser.add_argument("--out-dir", default="results_dinov3_bbch_station_role_sweep")
    ...
    parser.add_argument("--dense-features", dest="dense_features", action="store_true", default=True)
    parser.add_argument("--no-dense-features", dest="dense_features", action="store_false")
    parser.add_argument("--dense-grid-size", type=int, default=2)
    parser.add_argument("--dense-include-cls", dest="dense_include_cls", action="store_true", default=True)
    parser.add_argument("--no-dense-include-cls", dest="dense_include_cls", action="store_false")
    parser.add_argument("--image-backbone", default="facebook/dinov3-vitb16-pretrain-lvd1689m")
