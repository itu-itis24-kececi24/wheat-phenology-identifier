...
def load_tiling_config(checkpoint: Dict, embedding_cache: Optional[Dict], args) -> Dict:
    tiling = dict(checkpoint.get("tiling_config", {}))
...
def load_dense_feature_config(checkpoint: Dict, embedding_cache: Optional[Dict]) -> Dict:
    cache_config = dict(embedding_cache.get("dense_features", {})) if embedding_cache is not None else {}
    checkpoint_config = dict(checkpoint.get("dense_feature_config", {}))
    if cache_config and checkpoint_config and cache_config != checkpoint_config:
        raise ValueError(
            "Checkpoint and embedding cache use different dense feature settings: "
            f"checkpoint={checkpoint_config} cache={cache_config}"
        )
    return cache_config or checkpoint_config
...
def encode_one_image(
    ...
    dense_feature_config: Optional[Dict] = None,
) -> torch.Tensor:
    ...
    with Image.open(image_path) as image:
        ...
        for start in range(0, len(boxes), tile_batch_size):
            ...
            dense_feature_config = dense_feature_config or {}
            batch_features = (
                model.forward_dense(
                    batch,
                    grid_size=int(dense_feature_config.get("grid_size", 2)),
                    include_cls=bool(dense_feature_config.get("include_cls", True)),
                )
...
def pad_tile_features(features: List[torch.Tensor], feature_dim: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    dense_tokens = next((feat.shape[1] for feat in features if feat.ndim == 3), None)
    padded, masks = [], []
    for feat in features:
        ...
        if dense_tokens is not None:
            ...
            if feat.ndim != 3 or feat.shape[1] != dense_tokens:
                raise ValueError("All dense window features must have the same token count")
            out = torch.zeros(max_tiles, dense_tokens, feature_dim, dtype=torch.float32)
        else:
...
def temporal_feature_columns_from_checkpoint(checkpoint: Dict) -> List[str]:
    ...
    if checkpoint.get("use_weather_metadata", False):
        columns.extend(checkpoint.get("weather_feature_columns", WEATHER_TEMPORAL_FEATURE_COLUMNS))
...
def build_model(
    ...
    temporal_feature_fusion = str(checkpoint.get("temporal_feature_fusion", "legacy"))
    temporal_feature_gate_init = float(checkpoint.get("temporal_feature_gate_init", 0.1))
    weather_feature_dim = int(checkpoint.get("weather_feature_dim", 0))
    weather_feature_gate_init = float(checkpoint.get("weather_feature_gate_init", 0.1))
    location_feature_dim = int(checkpoint.get("location_feature_dim", 0))
    location_feature_hidden_dim = int(checkpoint.get("location_feature_hidden_dim", 16))
    location_gate_init = float(checkpoint.get("location_gate_init", 0.1))
    ...
    if stream == "both":
        model = MultiScaleEmbeddingTemporalTransformer(
            ...
            temporal_feature_fusion=temporal_feature_fusion,
            ...
            location_feature_hidden_dim=location_feature_hidden_dim,
            location_gate_init=location_gate_init,
...
def main():
    ...
    parser.add_argument("--image-backbone", default=None, help="Hugging Face model ID or local save_pretrained directory. Defaults to checkpoint/cache metadata.")
    parser.add_argument("--image-backbone-weights", default=None, help="Optional .pt/.pth image-backbone state dictionary.")
    parser.add_argument("--pretrained", action="store_true", default=True, help="Use pretrained DINOv3 if no embedding cache is supplied.")
    ...
    dense_feature_config = load_dense_feature_config(checkpoint, embedding_cache)
    feature_dim = (
        int(embedding_cache["feature_dim"])
        if embedding_cache is not None
        else int(checkpoint.get("embedding_feature_dim") or 768)
    )
    ...
    if args.image_backbone_weights:
        backbone_payload, weights_backbone = read_backbone_checkpoint(args.image_backbone_weights)
    backbone = args.image_backbone or weights_backbone or (
        embedding_cache.get("image_backbone")
        if embedding_cache is not None
        else checkpoint.get("image_backbone")
    ) or DINO_DEFAULT_BACKBONE
    pretrained = False if args.image_backbone_weights else (
        bool(embedding_cache.get("pretrained", args.pretrained))
        if embedding_cache is not None
        else bool(checkpoint.get("pretrained", args.pretrained))
    )

    local_config = infer_dinov3_config(backbone_payload) if backbone_payload is not None else None
    ...
    if args.image_backbone_weights:
        load_backbone_weights(encoder, args.image_backbone_weights, payload=backbone_payload)
    encoder.eval()
    preprocess = (
        embedding_cache.get("preprocess", {})
        if embedding_cache is not None
        else checkpoint.get("preprocess_config", {})
    )
    ...
    model = build_model(checkpoint, feature_dim, classes, stream, device, dense_feature_config)
    ...
    location_feature_dim = int(checkpoint.get("location_feature_dim", 0))
    if location_feature_dim > 0:
        if not station_code:
            raise ValueError(
                "Checkpoint expects location metadata; provide --station-code or use a filename containing the station code."
            )
        location_features = station_location_features(station_code, strict=True).unsqueeze(0).to(device)
        if location_features.shape[-1] != location_feature_dim:
            raise ValueError(
                f"Checkpoint expects {location_feature_dim} location features, got {location_features.shape[-1]}"
            )
    else:
    ...
    result = {
        ...
        "station_code": station_code,
        "use_location_metadata": bool(location_feature_dim > 0),
        "weather_feature_set": checkpoint.get("weather_feature_set", "full"),
        "weather_feature_columns": checkpoint.get("weather_feature_columns", []),
        "temporal_feature_fusion": checkpoint.get("temporal_feature_fusion", "legacy"),
        "location_feature_columns": checkpoint.get("location_feature_columns", list(LOCATION_FEATURE_COLUMNS) if location_feature_dim else []),
