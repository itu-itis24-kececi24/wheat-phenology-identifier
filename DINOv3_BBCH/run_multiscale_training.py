...
# Edit these values to change the fold split for DINOv3 experiments.
...
def location_gate_value(model):
    module = getattr(model, "_orig_mod", model)
    gate_logit = getattr(module, "location_gate_logit", None)
    if gate_logit is None:
        return None
    return float(torch.sigmoid(gate_logit.detach().float()).cpu().item())


def temporal_metadata_gate_values(model):
    module = getattr(model, "_orig_mod", model)
    values = {}
    for output_name, parameter_name in (
        ("temporal_feature_gate", "temporal_feature_gate_logit"),
        ("weather_feature_gate", "weather_feature_gate_logit"),
    ):
        gate_logit = getattr(module, parameter_name, None)
        if gate_logit is not None:
            values[output_name] = float(
                torch.sigmoid(gate_logit.detach().float()).cpu().item()
            )
    return values
...
def forward_model(model, batch: Dict):
    ...
    location_features = batch.get("location_features")
...
def build_model(args, cfg, embedding_cache, device, classes):
    ...
    weather_feature_dim = cfg.weather_feature_dim
    location_feature_dim = cfg.location_feature_dim
    dense_feature_config = embedding_cache.get("dense_features", {}) if embedding_cache is not None else {}
    dense_tokens_per_tile = (
        int(dense_feature_config.get("tokens_per_tile", 0))
        if dense_feature_config.get("enabled", False)
        else 0
    )
    if embedding_cache is None:
        if args.stream == "both":
            log("Initializing full two-stream image model")
            return MultiScaleTemporalTransformer(
                ...
                temporal_feature_fusion=args.temporal_feature_fusion,
                temporal_feature_gate_init=args.temporal_feature_gate_init,
                weather_feature_gate_init=args.weather_feature_gate_init,
                location_feature_dim=location_feature_dim,
                location_feature_hidden_dim=args.location_feature_hidden_dim,
                location_gate_init=args.location_gate_init,
    ...
    if args.stream == "both":
        log("Initializing cached two-stream embedding model")
        return MultiScaleEmbeddingTemporalTransformer(
            ...
            temporal_norm_first=args.temporal_norm_first,
            temporal_ffn_multiplier=args.temporal_ffn_multiplier,
            dense_tokens_per_tile=dense_tokens_per_tile,
        ).to(device)
...
def checkpoint_tiling_config(embedding_cache):
    if embedding_cache is None:
        return {}
    tiling = embedding_cache.get("tiling", {})
    keys = ("tile_streams", "tile_size", "tile_stride", "max_tiles", "vit_image_size", "tile_pooling")
    return {key: tiling[key] for key in keys if key in tiling}
...
def main():
    ...
    parser.add_argument("--out-dir", default="results_dinov3_bbch")
    ...
    parser.add_argument("--folds", type=int, default=5, help="Number of sampled folds for --fold-strategy random. Ignored by LOSO, which creates one test fold per station.")
    parser.add_argument("--fold-strategy", choices=["random", "loso"], default="random", help="random samples group combinations; loso tests every station exactly once and balances validation appearances.")
    parser.add_argument("--only-fold", type=int, default=None, help="Train/evaluate only this one-based generated fold while preserving its original fold ID. Required for fold-specific fine-tuned backbone caches.")
    ...
    parser.add_argument("--expected-stations", type=int, default=None, help="Fail before training unless metadata contains exactly this many physical stations. Useful for auditing LOSO runs.")
    ...
    parser.add_argument("--temporal-feature-hidden-dim", type=int, default=32, help="Hidden size for the calendar and weather metadata MLPs. Use 0 for one Linear layer per branch.")
    parser.add_argument("--temporal-feature-fusion", choices=["gated", "legacy"], default="gated", help="gated preserves the visual embedding with residual metadata branches; legacy reproduces the old concatenation bottleneck.")
    parser.add_argument("--temporal-feature-gate-init", type=float, default=0.1, help="Initial residual gate for days-since-planting metadata.")
    parser.add_argument("--use-location-metadata", action="store_true", help="Fuse normalized city-level latitude, longitude, and elevation after temporal aggregation.")
    parser.add_argument("--location-feature-hidden-dim", type=int, default=16, help="Hidden size of the static location/elevation MLP.")
    parser.add_argument("--location-gate-init", type=float, default=0.1, help="Initial contribution of the gated location branch; must be strictly between 0 and 1.")
    parser.add_argument("--use-weather-metadata", action="store_true", help="Fetch/cache Meteostat weather and fuse the selected GDD feature set through a gated residual MLP.")
    parser.add_argument("--weather-feature-set", choices=sorted(WEATHER_FEATURE_SETS), default="cumulative", help="Weather inputs: cumulative GDD only (default), daily+cumulative GDD, or the legacy full weather vector.")
    parser.add_argument("--weather-feature-gate-init", type=float, default=0.1, help="Initial residual contribution of the weather/GDD branch.")
    ...
    if not 0.0 < args.temporal_feature_gate_init < 1.0:
        parser.error("--temporal-feature-gate-init must be strictly between 0 and 1")
    if not 0.0 < args.weather_feature_gate_init < 1.0:
        parser.error("--weather-feature-gate-init must be strictly between 0 and 1")
    if args.location_feature_hidden_dim <= 0:
        parser.error("--location-feature-hidden-dim must be positive")
    if not 0.0 < args.location_gate_init < 1.0:
        parser.error("--location-gate-init must be strictly between 0 and 1")
    ...
    log(
        f"Temporal metadata fusion: {args.temporal_feature_fusion} "
        f"calendar_gate_init={args.temporal_feature_gate_init:g}"
    )
    log(
        "Location/elevation metadata: "
        + (
            f"enabled hidden_dim={args.location_feature_hidden_dim} gate_init={args.location_gate_init:g}"
            if args.use_location_metadata
            else "disabled"
        )
    )
    ...
    log(
        "Weather metadata: "
        + (
            f"enabled feature_set={args.weather_feature_set} "
            f"gate_init={args.weather_feature_gate_init:g}"
    ...
    if args.resume_checkpoint:
        ...
        resume_fold_strategy = resume_state.get("fold_strategy", "random")
        if resume_fold_strategy != args.fold_strategy:
            raise ValueError(
                f"Resume checkpoint uses fold_strategy={resume_fold_strategy}, but current value is "
                f"{args.fold_strategy}. Use --fold-strategy {resume_fold_strategy}."
            )
        ...
        resume_feature_fusion = str(resume_state.get("temporal_feature_fusion", "legacy"))
        if resume_feature_fusion != args.temporal_feature_fusion:
            raise ValueError(
                "Resume checkpoint temporal metadata fusion differs. Resume with "
                f"--temporal-feature-fusion {resume_feature_fusion}."
            )
        if resume_feature_fusion == "gated":
            resume_temporal_gate = float(resume_state.get("temporal_feature_gate_init", 0.1))
            resume_weather_gate = float(resume_state.get("weather_feature_gate_init", 0.1))
            if not math.isclose(
                resume_temporal_gate,
                args.temporal_feature_gate_init,
                rel_tol=0.0,
                abs_tol=1e-12,
            ):
                raise ValueError(
                    "Resume checkpoint calendar metadata gate differs. Resume with "
                    f"--temporal-feature-gate-init {resume_temporal_gate}."
                )
            if not math.isclose(
                resume_weather_gate,
                args.weather_feature_gate_init,
                rel_tol=0.0,
                abs_tol=1e-12,
            ):
                raise ValueError(
                    "Resume checkpoint weather metadata gate differs. Resume with "
                    f"--weather-feature-gate-init {resume_weather_gate}."
                )
        resume_uses_weather = bool(resume_state.get("use_weather_metadata", False))
        if resume_uses_weather != args.use_weather_metadata:
            required_flag = "--use-weather-metadata" if resume_uses_weather else "without --use-weather-metadata"
            raise ValueError(f"Resume checkpoint weather metadata setting differs. Resume {required_flag}.")
        if resume_uses_weather:
            resume_weather_set = str(resume_state.get("weather_feature_set", "full"))
            if resume_weather_set != args.weather_feature_set:
                raise ValueError(
                    "Resume checkpoint weather feature set differs. Resume with "
                    f"--weather-feature-set {resume_weather_set}."
                )
        resume_uses_location = bool(resume_state.get("use_location_metadata", False))
        if resume_uses_location != args.use_location_metadata:
            required_flag = "--use-location-metadata" if resume_uses_location else "without --use-location-metadata"
            raise ValueError(f"Resume checkpoint location metadata setting differs. Resume {required_flag}.")
        if resume_uses_location:
            resume_location_hidden = int(resume_state.get("location_feature_hidden_dim", 16))
            resume_location_gate = float(resume_state.get("location_gate_init", 0.1))
            if resume_location_hidden != args.location_feature_hidden_dim:
                raise ValueError(
                    "Resume checkpoint location_feature_hidden_dim differs. Resume with "
                    f"--location-feature-hidden-dim {resume_location_hidden}."
                )
            if not math.isclose(resume_location_gate, args.location_gate_init, rel_tol=0.0, abs_tol=1e-12):
                raise ValueError(
                    "Resume checkpoint location_gate_init differs. Resume with "
                    f"--location-gate-init {resume_location_gate}."
                )
    ...
    if args.tensorboard:
        if SummaryWriter is None:
            ...
            tb_writer.add_scalar("config/temporal_feature_gate_init", args.temporal_feature_gate_init, 0)
            tb_writer.add_scalar("config/weather_feature_gate_init", args.weather_feature_gate_init, 0)
            tb_writer.add_scalar("config/use_location_metadata", int(args.use_location_metadata), 0)
            tb_writer.add_scalar("config/location_feature_hidden_dim", args.location_feature_hidden_dim, 0)
            tb_writer.add_scalar("config/location_gate_init", args.location_gate_init, 0)
            ...
            log_tb_text(
                tb_writer,
                "config/metadata_fusion",
                {
                    "temporal_feature_fusion": args.temporal_feature_fusion,
                    "weather_feature_set": args.weather_feature_set,
                },
            )
    ...
    if args.use_weather_metadata:
        ...
        weather_feature_columns = tuple(WEATHER_FEATURE_SETS[args.weather_feature_set])
        temporal_feature_columns.extend(weather_feature_columns)
        log(
            "Weather features supplied to the model: "
            + ", ".join(weather_feature_columns)
        )
    location_feature_columns = tuple()
    if args.use_location_metadata:
        daily_df = add_location_metadata(daily_df, strict=True)
        location_feature_columns = tuple(LOCATION_FEATURE_COLUMNS)
        location_table = (
            daily_df[
                [
                    "station_code",
        ...
        log("Location/elevation table used by the model:\n" + location_table.to_string(index=False))
    ...
    if tb_writer is not None:
        ...
        tb_writer.add_scalar("config/weather_feature_dim", len(weather_feature_columns), 0)
        tb_writer.add_scalar("config/location_feature_dim", len(location_feature_columns), 0)
        log_tb_text(tb_writer, "config/temporal_feature_columns", {"columns": list(temporal_feature_columns)})
        log_tb_text(tb_writer, "config/weather_feature_columns", {"columns": list(weather_feature_columns)})
        log_tb_text(tb_writer, "config/location_feature_columns", {"columns": list(location_feature_columns)})
    ...
    print_station_image_edges(
        ...
        printer=log,
        title="First/last resolved images used for DINOv3 training:",
    )
    if tb_writer is not None:
    ...
    total_groups = int(daily_df[split_group_col].nunique())
    if args.expected_stations is not None:
        if args.fold_group_by != "station":
            raise ValueError("--expected-stations requires --fold-group-by station.")
        if total_groups != int(args.expected_stations):
            discovered = sorted(daily_df["station_code"].dropna().astype(str).unique().tolist())
            raise ValueError(
                f"Expected {args.expected_stations} stations, but metadata contains {total_groups}: "
                f"{discovered}"
            )
    n_val_groups = int(VALIDATION_FOLD_STATIONS if args.validation_groups is None else args.validation_groups)
    if args.fold_strategy == "loso":
        if args.fold_group_by != "station":
            raise ValueError("LOSO requires --fold-group-by station so every physical station is tested once.")
        if args.test_groups is not None and int(args.test_groups) != 1:
            raise ValueError("LOSO always uses one test station; omit --test-groups or set --test-groups 1.")
        n_test_groups = 1
    else:
    ...
    if n_train_groups < 1:
        raise ValueError(
            "Invalid DINOv3 split constants: "
    ...
    log(f"Fold strategy: {args.fold_strategy}")
    log(f"DINOv3 fold station counts: train={n_train_groups} val={n_val_groups} test={n_test_groups}")
    log(
        "DINOv3 fold station diversity requirements: "
    ...
    if args.fold_strategy == "loso":
        ...
        log(
            f"Generated {len(folds)} LOSO folds: every station is test once "
            f"and validation {n_val_groups} time(s). --folds={args.folds} was ignored."
        )
    else:
    ...
    if args.only_fold is not None and not (1 <= args.only_fold <= len(folds)):
        raise ValueError(f"--only-fold must be in 1..{len(folds)}, got {args.only_fold}")
    fold_items = list(enumerate(folds, 1))
    if args.only_fold is not None:
        fold_items = [item for item in fold_items if item[0] == args.only_fold]
        log(f"Restricting this run to generated fold {args.only_fold}/{len(folds)}")
    ...
    cfg = WindowConfig(
        ...
        location_feature_columns=location_feature_columns,
    )
    log(
        ...
        f"| weather_feature_dim={cfg.weather_feature_dim} "
        f"| location_feature_dim={cfg.location_feature_dim}"
    )
    all_history = []
    all_test_metrics = []
    all_val_metrics = []
    ...
    if args.embedding_cache:
        ...
        dense_feature_config = dict(embedding_cache.get("dense_features", {}))
        effective_image_backbone = str(embedding_cache.get("image_backbone", args.image_backbone))
        effective_pretrained = bool(embedding_cache.get("pretrained", args.pretrained))
        if dense_feature_config.get("enabled", False):
            if embedding_cache.get("tiling", {}).get("tile_pooling") != "attention":
                raise ValueError("Dense DINOv3 caches require tile_pooling=attention")
            log(
                "Dense DINOv3 cache: "
                f"grid={dense_feature_config.get('grid_size')} "
                f"include_cls={dense_feature_config.get('include_cls')} "
                f"tokens_per_tile={dense_feature_config.get('tokens_per_tile')}"
            )
        if resume_state is not None:
            resume_dense = dict(resume_state.get("dense_feature_config", {}))
            if resume_dense and resume_dense != dense_feature_config:
                raise ValueError(
                    "Resume checkpoint dense feature configuration does not match the embedding cache: "
                    f"checkpoint={resume_dense} cache={dense_feature_config}"
                )
    else:
        embedding_cache = None
        dense_feature_config = {}
        effective_image_backbone = args.image_backbone
        effective_pretrained = args.pretrained
    ...
    for fold_id, (train_idx, val_idx, test_idx) in fold_items:
        ...
        for epoch in range(start_epoch, args.epochs + 1):
            ...
            learned_location_gate = location_gate_value(model)
            if learned_location_gate is not None:
                row["location_gate"] = learned_location_gate
            learned_metadata_gates = temporal_metadata_gate_values(model)
            row.update(learned_metadata_gates)
            history.append(row)
            ...
            if tb_writer is not None:
                ...
                if learned_location_gate is not None:
                    tb_writer.add_scalar(
                        f"fold_{fold_id}/model/location_gate",
                        learned_location_gate,
                        epoch,
                    )
                for gate_name, gate_value in learned_metadata_gates.items():
                    tb_writer.add_scalar(
                        f"fold_{fold_id}/model/{gate_name}",
                        gate_value,
                        epoch,
                    )
                if device.type == "cuda":
                ...
                torch.save(
                    {
                        ...
                        "target_index": cfg.center,
                        "stream": args.stream,
                        "image_backbone": effective_image_backbone,
                        ...
                        "weather_feature_set": args.weather_feature_set,
                        "use_location_metadata": args.use_location_metadata,
                        "temporal_feature_dim": cfg.temporal_feature_dim,
                        ...
                        "temporal_feature_gate_init": args.temporal_feature_gate_init,
                        "weather_feature_dim": cfg.weather_feature_dim,
                        "weather_feature_columns": list(cfg.weather_feature_columns),
                        "weather_feature_gate_init": args.weather_feature_gate_init,
                        "location_feature_dim": cfg.location_feature_dim,
                        "location_feature_columns": list(cfg.location_feature_columns),
                        "location_feature_hidden_dim": args.location_feature_hidden_dim,
                        "location_gate_init": args.location_gate_init,
                        "gdd_base_temp": args.gdd_base_temp,
                        ...
                        "embedding_feature_dim": int(embedding_cache["feature_dim"]) if embedding_cache is not None else None,
                        "tiling_config": checkpoint_tiling_config(embedding_cache),
                        "preprocess_config": dict(embedding_cache.get("preprocess", {})) if embedding_cache is not None else {},
                        "dense_feature_config": dense_feature_config,
        ...
        best_metadata_gates = temporal_metadata_gate_values(model)
        best_val_row.update(best_metadata_gates)
        best_location_gate = location_gate_value(model)
        if best_location_gate is not None:
            best_val_row["location_gate"] = best_location_gate
            with open(val_metrics_path, "w", encoding="utf-8") as f:
                json.dump(best_val_row, f, indent=2)
            log(f"Fold {fold_id}: learned location gate at best epoch={best_location_gate:.4f}")
        if best_metadata_gates:
            with open(val_metrics_path, "w", encoding="utf-8") as f:
                json.dump(best_val_row, f, indent=2)
            log(
                f"Fold {fold_id}: learned metadata gates at best epoch="
                + ", ".join(f"{name}={value:.4f}" for name, value in best_metadata_gates.items())
            )
        ...
        if test_loader is None:
            ...
            if best_location_gate is not None:
                test_row["location_gate"] = best_location_gate
            test_row.update(best_metadata_gates)
