...
except Exception:  # pragma: no cover - optional DINOv2 dependency
    AutoConfig = None
...
DINO_DEFAULT_BACKBONE = "facebook/dinov2-base"
...
# Newer shifted labeling: keep 1-Ekim as planting metadata, but start visible
...
def _extract_capture_timestamp(path_or_name: str) -> Optional[pd.Timestamp]:
    """Return a capture timestamp from the repository's timestamped filenames."""
...
def _daily_path_sort_key(path: str) -> Tuple[bool, pd.Timestamp, str]:
    """Put timestamped files first, ordered by capture time then path."""
...
def _select_daily_image_paths(candidates: Dict[str, List[str]]) -> Dict[str, str]:
    """Select one deterministic daily path per scale.

    With a single available scale, the earliest capture is kept. With both 1X
    (macro) and 10X (micro) candidates, select the pair with the smallest
    capture-time difference so the two views describe nearly the same crop
    state. Ties prefer the earliest pair. Files without parsable timestamps
    fall back to stable path ordering.
    """
    available = {
    ...
    if not macro_paths or not micro_paths:
        return {
            scale: paths[0]
            for scale, paths in available.items()
        }
    ...
    # No complete timestamp pair: preserve deterministic earliest-per-stream behavior.
...
def add_weather_metadata(
    ...
    weather_df["date"] = pd.to_datetime(weather_df["date"], errors="coerce").dt.normalize()
    weather_df["station_family"] = weather_df["station_family"].astype(str)
    ...
    merge_cols = ["station_family", "date"] + numeric_cols
    ...
    for _, group_idx in out.groupby("group_id").groups.items():
        ...
        gdd = group["weather_gdd_raw"].copy()
        if planting_date is not None:
            gdd = gdd.where(group["date"] >= planting_date, 0.0)
        cumulative_gdd = gdd.cumsum().to_numpy()
...
class MultiScaleEmbeddingWindowDataset(Dataset):
    ...
    def _build_samples(self) -> List[Tuple[str, pd.Timestamp]]:
        samples = []
        skipped_missing_target_cache = 0
        for station_year, group in self.groups.items():
            for date, row in group.iterrows():
                if self.config.require_center_image and not self._has_required_stream(row):
                    skipped_missing_target_cache += 1
                    continue
                samples.append((station_year, pd.Timestamp(date)))
        if skipped_missing_target_cache:
            print(
                f"Warning: skipped {skipped_missing_target_cache} target-day samples "
    ...
    def _load_embedding(self, path: Optional[str], stream: str, aug_index: int = 0) -> torch.Tensor:
        key = _path_key(path)
        if key is None:
            return torch.zeros(self.max_tiles, self.feature_dim) if self.tile_attention else torch.zeros(self.feature_dim)
        ...
        if not self.tile_attention:
            return value.mean(dim=0) if value.ndim == 2 else value
        if value.ndim == 1:
    ...
    def __getitem__(self, idx: int):
        ...
        for step in range(self.config.window_days):
            ...
            macro_tile_masks.append(self._tile_mask(macro_input_path, "macro"))
            micro_tile_masks.append(self._tile_mask(micro_input_path, "micro"))
            if row is None:
                mask.append(0.0)
            elif self.config.stream == "micro":
                mask.append(float(present and pd.notna(row.get("micro_path"))))
            elif self.config.stream == "macro":
                mask.append(float(present and pd.notna(row.get("macro_path"))))
            else:
                mask.append(float(present and pd.notna(row.get("macro_path")) and pd.notna(row.get("micro_path"))))
...
class SMOTEEmbeddingWindowDataset(Dataset):
    """Create train-fold-only SMOTE windows from cached temporal embeddings."""
    ...
    def __init__(
        ...
        if target_ratio <= 0 or k_neighbors < 1:
            raise ValueError("SMOTE target_ratio must be positive and k_neighbors must be at least one")
    ...
    def _center_row(self, dataset_index: int) -> pd.Series:
        ...
        row = self.base_dataset.groups[station_year].loc[center_date]
        return row.iloc[0] if isinstance(row, pd.DataFrame) else row

    def _label_and_descriptor(self, dataset_index: int) -> Tuple[int, np.ndarray]:
        row = self._center_row(dataset_index)
        features = []
        ...
        label = int(torch.argmax(_target_to_tensor(row["target"])).item())
        return label, torch.cat(features).float().cpu().numpy()

    def _build_synthetic_specs(self) -> Dict[str, object]:
        try:
            from sklearn.neighbors import NearestNeighbors
        except ImportError as exc:
            raise ImportError("SMOTE requires scikit-learn. Run: python -m pip install scikit-learn") from exc

        # Cache variants count for balance but not neighbour search: they are
        # alternate renderings of the same biological day-window.
        source_indices = np.arange(len(self.base_dataset.samples), dtype=np.int64) * self.base_dataset.sample_multiplier
        labels, descriptors = [], []
        for index in source_indices:
            label, descriptor = self._label_and_descriptor(int(index))
            labels.append(label)
            descriptors.append(descriptor)
        labels = np.asarray(labels, dtype=np.int64)
        descriptors = np.stack(descriptors) if descriptors else np.empty((0, 0), dtype=np.float32)
        class_counts = {
            int(label): int((labels == label).sum()) * self.base_dataset.sample_multiplier
            for label in np.unique(labels)
        }
        largest = max(class_counts.values(), default=0)
        requested = {label: max(0, int(math.ceil(largest * self.target_ratio)) - count) for label, count in class_counts.items()}
        if self.max_synthetic_samples > 0 and sum(requested.values()) > self.max_synthetic_samples:
            scale = self.max_synthetic_samples / sum(requested.values())
            requested = {label: int(math.floor(count * scale)) for label, count in requested.items()}

        rng = np.random.default_rng(self.seed)
        generated, skipped = {}, {}
        for label, requested_count in requested.items():
            positions = np.flatnonzero(labels == label)
            if requested_count == 0:
                continue
            if len(positions) < 2:
                skipped[label] = "fewer than two training windows"
                continue
            neighbors = NearestNeighbors(
                n_neighbors=min(self.k_neighbors + 1, len(positions)), metric="cosine"
            ).fit(descriptors[positions])
            neighbor_positions = neighbors.kneighbors(descriptors[positions], return_distance=False)
            for _ in range(requested_count):
                source_position = int(rng.integers(len(positions)))
                ...
                self.synthetic_specs.append(
                    (
                        int(source_indices[positions[source_position]]),
                        int(source_indices[positions[neighbor_position]]),
        ...
        return {
            ...
            "requested_synthetic_by_class": requested,
            "generated_synthetic_by_class": generated,
    ...
    def __getitem__(self, idx: int):
        ...
        source, neighbor = self.base_dataset[source_idx], self.base_dataset[neighbor_idx]
        output = dict(source)
        for key in ("macro", "micro", "temporal_features", "target", "date_score"):
            output[key] = source[key] + (neighbor[key] - source[key]) * interpolation
...
class ViTBackboneFeatureExtractor(nn.Module):
    def __init__(self, backbone: str = DINO_DEFAULT_BACKBONE, pretrained: bool = True):
        super().__init__()
        ...
        if self.backbone_source == "huggingface":
            if AutoConfig is None or AutoImageProcessor is None or AutoModel is None:
                raise ImportError("transformers is required for DINOv2/Hugging Face backbones")
            self.processor = AutoImageProcessor.from_pretrained(self.backbone_name)
            config = AutoConfig.from_pretrained(self.backbone_name)
            self.backbone = AutoModel.from_pretrained(self.backbone_name) if pretrained else AutoModel.from_config(config)
...
def _is_huggingface_backbone(backbone: str) -> bool:
    value = _normalize_backbone_name(backbone).lower()
    return "/" in value or value.startswith("dinov2")
...
class GatedMultiviewFusion(nn.Module):
    def __init__(
        ...
        gate_input_dim = input_dim * 2 + self.temporal_feature_dim
    ...
    def forward(
        ...
        if self.temporal_feature_dim > 0:
            if temporal_features is None:
                ...
                temporal_features = temporal_features.to(device=macro.device, dtype=macro.dtype)
            if temporal_features.shape[:2] != macro.shape[:2] or temporal_features.shape[-1] != self.temporal_feature_dim:
                raise ValueError(
                    "temporal_features must have shape "
                    f"[batch, days, {self.temporal_feature_dim}], got {tuple(temporal_features.shape)}"
                )
            gate_inputs.append(temporal_features)
        logits = self.gate(torch.cat(gate_inputs, dim=-1)).float()
        weights = torch.softmax(logits, dim=-1).to(dtype=macro.dtype)
        self.last_gate_weights = weights.detach()
        fused = (
            weights[..., 0:1] * self.macro_proj(macro)
            + weights[..., 1:2] * self.micro_proj(micro)
        )
        return self.post(fused)


class CrossAttentionTileFusion(nn.Module):
    def __init__(
        self,
        input_dim: int,
        embed_dim: int,
        ...
        self.grid_aspect_ratio = grid_aspect_ratio
        self.temporal_feature_dim = int(temporal_feature_dim)
        self.macro_proj = nn.Sequential(nn.LayerNorm(input_dim), nn.Linear(input_dim, embed_dim))
        self.micro_proj = nn.Sequential(nn.LayerNorm(input_dim), nn.Linear(input_dim, embed_dim))
        self.row_pos = nn.Parameter(torch.zeros(1, max_rows, embed_dim))
        self.col_pos = nn.Parameter(torch.zeros(1, max_cols, embed_dim))
        nn.init.trunc_normal_(self.row_pos, std=0.02)
        nn.init.trunc_normal_(self.col_pos, std=0.02)
        self.macro_to_micro = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        self.micro_to_macro = nn.MultiheadAttention(embed_dim, num_heads, dropout=dropout, batch_first=True)
        self.macro_norm = nn.LayerNorm(embed_dim)
        self.micro_norm = nn.LayerNorm(embed_dim)
        ...
        self.vector_fusion = GatedMultiviewFusion(
            input_dim=embed_dim,
            embed_dim=embed_dim,
    ...
    def _tile_position(self, tile_count: int, device: torch.device) -> torch.Tensor:
        cols = max(1, math.ceil(math.sqrt(tile_count * self.grid_aspect_ratio)))
        rows = math.ceil(tile_count / cols)
        if rows > self.max_rows or cols > self.max_cols:
            raise ValueError(
        ...
        tile_index = torch.arange(tile_count, device=device)
        row_index = torch.div(tile_index, cols, rounding_mode="floor")
        col_index = tile_index.remainder(cols)
        return self.row_pos[:, row_index, :] + self.col_pos[:, col_index, :]

    def _default_mask(self, features: torch.Tensor, mask: Optional[torch.Tensor]) -> torch.Tensor:
        if mask is not None:
            return mask.bool()
        return torch.ones(features.shape[:-1], dtype=torch.bool, device=features.device)

    def _attention_pool(self, tokens: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        logits = self.pool_score(tokens).squeeze(-1).float()
        valid = mask.bool()
        logits = logits.masked_fill(~valid, torch.finfo(logits.dtype).min)
        empty = ~valid.any(dim=-1, keepdim=True)
        logits = torch.where(empty, torch.zeros_like(logits), logits)
        ...
        return (tokens * weights.unsqueeze(-1).to(dtype=tokens.dtype)).sum(dim=-2)
    ...
    def forward(
        ...
        if macro.ndim == 3 and micro.ndim == 3:
            fused = self.vector_fusion(
                self.macro_proj(macro),
                self.micro_proj(micro),
                temporal_features,
            )
            self.last_gate_weights = self.vector_fusion.last_gate_weights
            return fused
        if macro.ndim != 4 or micro.ndim != 4:
            raise ValueError(
                "CrossAttentionTileFusion expects both streams as [batch, days, tiles, features] "
                f"or [batch, days, features], got macro={tuple(macro.shape)} micro={tuple(micro.shape)}"
            )

        bsz, days, macro_tiles, _ = macro.shape
        _, _, micro_tiles, _ = micro.shape
        macro_mask = self._default_mask(macro, macro_tile_mask).reshape(bsz * days, macro_tiles)
        micro_mask = self._default_mask(micro, micro_tile_mask).reshape(bsz * days, micro_tiles)
        macro_tokens = self.macro_proj(macro).reshape(bsz * days, macro_tiles, -1)
        micro_tokens = self.micro_proj(micro).reshape(bsz * days, micro_tiles, -1)
        macro_tokens = macro_tokens + self._tile_position(macro_tiles, macro.device).to(dtype=macro_tokens.dtype)
        micro_tokens = micro_tokens + self._tile_position(micro_tiles, micro.device).to(dtype=micro_tokens.dtype)

        macro_empty = ~macro_mask.any(dim=-1, keepdim=True)
        micro_empty = ~micro_mask.any(dim=-1, keepdim=True)
        macro_key_padding = torch.where(
            macro_empty.expand_as(macro_mask),
            torch.zeros_like(macro_mask, dtype=torch.bool),
            ~macro_mask,
        )
        micro_key_padding = torch.where(
            micro_empty.expand_as(micro_mask),
            torch.zeros_like(micro_mask, dtype=torch.bool),
            ~micro_mask,
        )

        macro_cross, _ = self.macro_to_micro(
            query=macro_tokens,
            key=micro_tokens,
            value=micro_tokens,
            key_padding_mask=micro_key_padding,
            need_weights=False,
        )
        micro_cross, _ = self.micro_to_macro(
            query=micro_tokens,
            key=macro_tokens,
            value=macro_tokens,
            key_padding_mask=macro_key_padding,
            need_weights=False,
        )
        macro_context = self.macro_norm(macro_tokens + macro_cross)
        micro_context = self.micro_norm(micro_tokens + micro_cross)
        macro_vec = self._attention_pool(macro_context, macro_mask).reshape(bsz, days, -1)
        micro_vec = self._attention_pool(micro_context, micro_mask).reshape(bsz, days, -1)
        fused = self.vector_fusion(macro_vec, micro_vec, temporal_features)
        self.last_gate_weights = self.vector_fusion.last_gate_weights
        return fused
...
class MultiScaleTemporalTransformer(nn.Module, TemporalAggregationMixin):
    def __init__(
        ...
        self._init_temporal_features(embed_dim, temporal_feature_dim, dropout, temporal_feature_hidden_dim)
        ...
        self.fusion = GatedMultiviewFusion(
            ...
            embed_dim=embed_dim,
            temporal_feature_dim=temporal_feature_dim,
            hidden_dim=temporal_feature_hidden_dim if temporal_feature_hidden_dim > 0 else embed_dim,
            dropout=dropout,
        )
        self._init_temporal_model(embed_dim, temporal_layers, temporal_heads, dropout, temporal_model)
    ...
    def forward(
        ...
        x = self.fusion(macro_feat, micro_feat, temporal_features)
        ...
        return self.classifier(self._aggregate_temporal(x, mask, target_index))
...
class MultiScaleEmbeddingTemporalTransformer(nn.Module, TemporalAggregationMixin):
    def __init__(
        ...
        self._init_temporal_aggregation(embed_dim, temporal_aggregation)
        self._init_temporal_features(embed_dim, temporal_feature_dim, dropout, temporal_feature_hidden_dim)
        self.fusion = CrossAttentionTileFusion(
    ...
    def forward(
        ...
        x = self.fusion(macro, micro, macro_tile_mask, micro_tile_mask, temporal_features)
        x = self._add_temporal_features(x, temporal_features)
        x, mask = self._run_temporal_model(x, mask)
