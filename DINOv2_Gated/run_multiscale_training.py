...
# Edit these values to change the fold split for DINOv2 gated-fusion experiments.
...
def main():
    ...
    parser.add_argument("--modality-dropout", type=float, default=0.0, help="During gated two-stream training, randomly hide one available view on this fraction of day tokens.")
    ...
    log(f"Gated modality dropout: {args.modality_dropout}")
    log(f"Temporal metadata hidden dim: {args.temporal_feature_hidden_dim}")
    log("Two-stream fusion: gated macro/micro weighting when --stream both is used")
