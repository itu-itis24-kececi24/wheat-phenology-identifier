...
def infer_date(args) -> pd.Timestamp:
    ...
    raise ValueError("Provide --target-date, or use a target image whose filename contains a date.")
...
def main():
    ...
    parser.add_argument("--image-path", default=None, help="Target image for a single-stream checkpoint.")
    parser.add_argument("--image-dir", default=None, help="Previous-day folder for a single-stream checkpoint.")
    parser.add_argument("--macro-path", default=None, help="Target-day 1X image for a gated two-stream checkpoint.")
    parser.add_argument("--micro-path", default=None, help="Target-day 10X image for a gated two-stream checkpoint.")
    parser.add_argument("--macro-dir", default=None, help="Folder containing previous 1X images.")
    parser.add_argument("--micro-dir", default=None, help="Folder containing previous 10X images.")
    ...
    station_code = args.station_code or parse_station_code(args.image_path or args.micro_path or args.macro_path)
    ...
    if stream == "both":
        if args.image_path:
            raise ValueError("For --stream both, provide separate --macro-path and --micro-path instead of --image-path.")
        if not args.macro_path or not args.micro_path:
            raise ValueError("Gated two-stream inference requires both --macro-path and --micro-path for the target day.")
        macro_window_paths = build_window_paths(args.macro_path, args.macro_dir, target_date, window_days, "macro")
        micro_window_paths = build_window_paths(args.micro_path, args.micro_dir, target_date, window_days, "micro")
    else:
        target_path = args.image_path or (args.micro_path if stream == "micro" else args.macro_path)
        image_dir = args.image_dir or (args.micro_dir if stream == "micro" else args.macro_dir)
        if not target_path:
            raise ValueError("Provide --image-path for single-stream inference.")
        single_window = build_window_paths(target_path, image_dir, target_date, window_days, stream)
        macro_window_paths = single_window if stream == "macro" else [None] * window_days
        micro_window_paths = single_window if stream == "micro" else [None] * window_days
    if args.debug_window:
        start_date = target_date - pd.Timedelta(days=window_days - 1)
        print("Resolved inference window:", flush=True)
        for offset, (macro_path, micro_path) in enumerate(zip(macro_window_paths, micro_window_paths)):
            date = start_date + pd.Timedelta(days=offset)
            print(f"  {date.date()} macro={macro_path or 'MISSING'} micro={micro_path or 'MISSING'}", flush=True)
    encoded_by_path = {}
    all_paths = sorted(
        {path_key(path) for path in macro_window_paths + micro_window_paths if path is not None}
    )
    ...
    def feature_sequence(paths, target_path):
        target_feature = encoded_by_path[path_key(target_path)]
        features, valid = [], []
        for path in paths:
            if path is not None:
                features.append(encoded_by_path[path_key(path)])
                valid.append(True)
            elif args.repeat_missing:
                features.append(target_feature)
                valid.append(True)
            else:
                features.append(torch.zeros_like(target_feature, dtype=torch.float32))
                valid.append(False)
        return features, valid

    if stream in {"macro", "both"}:
        macro_target = args.macro_path if stream == "both" else (args.image_path or args.macro_path)
        macro_features, macro_valid = feature_sequence(macro_window_paths, macro_target)
    else:
        reference = encoded_by_path[path_key(args.image_path or args.micro_path)]
        macro_features = [torch.zeros_like(reference, dtype=torch.float32) for _ in range(window_days)]
        macro_valid = [False] * window_days
    if stream in {"micro", "both"}:
        micro_target = args.micro_path if stream == "both" else (args.image_path or args.micro_path)
        micro_features, micro_valid = feature_sequence(micro_window_paths, micro_target)
    else:
        reference = encoded_by_path[path_key(args.image_path or args.macro_path)]
        micro_features = [torch.zeros_like(reference, dtype=torch.float32) for _ in range(window_days)]
        micro_valid = [False] * window_days
    mask = [macro or micro for macro, micro in zip(macro_valid, micro_valid)]
    ...
    if tiling.get("tile_pooling") == "attention":
        ...
        macro_tile_mask &= torch.tensor(macro_valid, dtype=torch.bool).unsqueeze(-1)
        micro_tile_mask &= torch.tensor(micro_valid, dtype=torch.bool).unsqueeze(-1)
        ...
        macro = torch.stack([feat.float() if feat.ndim == 1 else feat.float().mean(dim=0) for feat in macro_features]).unsqueeze(0).to(device)
        micro = torch.stack([feat.float() if feat.ndim == 1 else feat.float().mean(dim=0) for feat in micro_features]).unsqueeze(0).to(device)
        with torch.no_grad():
            if stream == "both":
                logits = model(
                    macro,
                    ...
                    macro_valid=torch.tensor(macro_valid, dtype=torch.bool, device=device).unsqueeze(0),
                    micro_valid=torch.tensor(micro_valid, dtype=torch.bool, device=device).unsqueeze(0),
                )
            else:
        ...
        "used_dates": [
            {
                ...
                "macro_path": os.path.abspath(macro_window_paths[i]) if macro_window_paths[i] else None,
                "micro_path": os.path.abspath(micro_window_paths[i]) if micro_window_paths[i] else None,
                "available": bool(mask[i]),
            }
