...
class MultiScaleWindowDataset(Dataset):
    ...
    def _has_required_stream(self, row: pd.Series) -> bool:
        ...
        return pd.notna(row.get("macro_path")) or pd.notna(row.get("micro_path"))
    ...
    def __getitem__(self, idx: int):
        ...
        for step in range(self.config.window_days):
            ...
            macro_valid.append(bool(present and macro_ok))
            micro_valid.append(bool(present and micro_ok))
            if row is None:
                mask.append(0.0)
                ...
                mask.append(float(present and (macro_ok or micro_ok)))
        ...
        return {
            ...
            "macro_valid": torch.tensor(macro_valid, dtype=torch.bool),
            "micro_valid": torch.tensor(micro_valid, dtype=torch.bool),
...
class MultiScaleEmbeddingWindowDataset(Dataset):
    ...
    def _has_required_stream(self, row: pd.Series) -> bool:
        ...
        return cached(row.get("macro_path"), self.macro_embeddings) or cached(row.get("micro_path"), self.micro_embeddings)
    ...
    def __getitem__(self, idx: int):
        ...
        return {
            ...
            "macro_valid": torch.stack(macro_tile_masks).any(dim=-1),
            "micro_valid": torch.stack(micro_tile_masks).any(dim=-1),
            "mask": torch.tensor(mask, dtype=torch.bool),
            "target": target,
            "date_score": date_score,
            "label": torch.tensor(label, dtype=torch.long),
...
class GatedMultiviewFusion(nn.Module):
    ...
    def forward(
        ...
        macro_valid: Optional[torch.Tensor] = None,
        micro_valid: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        ...
        macro_valid = (
            torch.ones(macro.shape[:2], dtype=torch.bool, device=macro.device)
            if macro_valid is None
            else macro_valid.to(device=macro.device, dtype=torch.bool)
        )
        micro_valid = (
            torch.ones(micro.shape[:2], dtype=torch.bool, device=micro.device)
            if micro_valid is None
            else micro_valid.to(device=micro.device, dtype=torch.bool)
        )
        if self.training and self.modality_dropout > 0.0:
            both_valid = macro_valid & micro_valid
            drop = both_valid & (torch.rand(macro_valid.shape, device=macro.device) < self.modality_dropout)
            drop_macro = drop & (torch.rand(macro_valid.shape, device=macro.device) < 0.5)
            macro_valid = macro_valid & ~drop_macro
            micro_valid = micro_valid & ~(drop & ~drop_macro)

        valid = torch.stack([macro_valid, micro_valid], dim=-1)
        ...
        weights = weights.to(dtype=macro.dtype)
        fused = (
...
class MultiScaleTemporalTransformer(nn.Module, TemporalAggregationMixin):
    def __init__(
        ...
        super().__init__()
        self.supports_modality_masks = True
    ...
    def forward(
        ...
        x = self.fusion(macro_feat, micro_feat, temporal_features, macro_valid, micro_valid)
...
class MultiScaleEmbeddingTemporalTransformer(nn.Module, TemporalAggregationMixin):
    ...
    def forward(
        ...
        if macro_valid is None and macro_tile_mask is not None:
            macro_valid = macro_tile_mask.bool().any(dim=-1)
        if micro_valid is None and micro_tile_mask is not None:
            micro_valid = micro_tile_mask.bool().any(dim=-1)
        macro = self.tile_pool(macro, macro_tile_mask)
        micro = self.tile_pool(micro, micro_tile_mask)
        x = self.fusion(macro, micro, temporal_features, macro_valid, micro_valid)
        x = self._add_temporal_features(x, temporal_features)
        x, mask = self._run_temporal_model(x, mask)
