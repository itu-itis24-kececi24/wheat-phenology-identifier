...
The active ViT2 pipeline is self-contained in `multiscale_phenology.py` for metadata creation, label mapping, station lookup, image pairing, `OffSeason` windows, soft labels, temporal windows, and model definitions.

## Architecture Overview

The project is a temporal classifier. By default in `ViT2`, it predicts the last day in a causal rolling window.
...
If `meteostat` is installed, ViT2 can append daily weather features to each timestep:
...
```powershell
--weather-cache results_vit2_weather\meteostat_weather_cache.csv
```
...
By default it uses `--camera AUTO` and trains on the `10X` stream. `AUTO` uses the label table `kamera` / `Camera` column when present; if the row has no camera value, it prefers `K1` and falls back to the next available camera. Use `--camera K1`, `--camera K2`, or `--camera ALL` to override the label table.
...
Option 1: run the helper script from `ViT2`:
...
```python
from ViT2.multiscale_phenology import build_multiscale_daily_dataframe
...
```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx ...
python ViT2\run_multiscale_training.py --label-path labeling_iso_dates.csv ...
```
...
```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_smoke_vit2 --epochs 1 --folds 1 --window-days 3 --window-mode causal --stream micro --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 1 --accumulation-steps 1 --num-workers 0 --camera AUTO --device cuda --log-interval 1
```
...
For `ViT2`, validation/test station counts are edited in [run_multiscale_training.py](run_multiscale_training.py), not passed as command-line arguments:
...
```powershell
python ViT2\precompute_multiscale_embeddings.py --excel-path labeling.xlsx --data-path data --out-dir results_vit2_cache --camera AUTO --stream micro --tile-streams micro --tile-pooling attention --tile-size 224 --tile-stride 224 --max-tiles 0 --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 64 --num-workers 4 --embedding-dtype float16 --pretrained --device cuda
```

Step 2: train from cached vectors:

```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_vit2_cache --embedding-cache results_vit2_cache\vit_embeddings.pt --epochs 5 --folds 1 --window-days 31 --window-mode causal --stream micro --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 32 --accumulation-steps 1 --num-workers 0 --camera AUTO --device cuda --log-interval 10
```

This is much faster because training no longer decodes JPEGs or runs ViT for every frame during every epoch.

### SMOTE For Cached Embeddings

`--embedding-smote` is an optional class-imbalance experiment for the cached workflow. It does not fabricate JPEGs. Within each training fold only, it selects two nearest 31-day cached windows from the same hard phenology stage and linearly interpolates their image embeddings, masks, time metadata, soft target, and date-aware score. Validation and test windows remain real, so the group-based evaluation stays valid.

Start conservatively. The following raises minority stages to 75% of the largest training class and caps synthetic windows at 2,000 per fold:

```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_vit2_smote --embedding-cache results_vit2_cache\vit_embeddings.pt --epochs 20 --folds 5 --window-days 31 --window-mode causal --stream micro --batch-size 32 --accumulation-steps 1 --num-workers 0 --camera AUTO --device cuda --embedding-smote --smote-target-ratio 0.75 --smote-k-neighbors 5 --smote-max-synthetic-samples 2000 --smote-seed 42
```

Use `--smote-target-ratio 1.0` to fully balance to the largest training class. Review each `fold_N\smote_summary.json` before relying on the result. SMOTE is intentionally unavailable for raw-image/full-image training because interpolation is meaningful in embedding space, not pixel space.
...
```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_vit2_full --epochs 20 --folds 5 --window-days 31 --window-mode causal --stream micro --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 2 --accumulation-steps 16 --num-workers 4 --camera AUTO --pretrained --device cuda --log-interval 25
```
...
```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_vit2_cache --embedding-cache results_vit2_cache\vit_embeddings.pt --epochs 5 --folds 1 --window-days 31 --window-mode causal --stream micro --device cuda --compile-model --compile-mode reduce-overhead
```
...
```powershell
python ViT2\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_vit2_cache --embedding-cache results_vit2_cache\vit_embeddings.pt --resume-checkpoint results_vit2_cache\last_checkpoint.pt --epochs 20 --device cuda
```

You can also resume from a fold-specific checkpoint:

```powershell
--resume-checkpoint results_vit2_cache\fold_3\last_checkpoint.pt
```
...
```powershell
tensorboard --logdir results_vit2_cache\tensorboard
```
...
```powershell
python ViT2\infer_cached_window.py --checkpoint results_vit2_cache\fold_1\best_model.pt --embedding-cache results_vit2_cache\vit_embeddings.pt --image-path "path\to\target_10X.jpeg" --planting-date 2023-11-15 --device cuda
```
...
```powershell
python ViT2\infer_cached_window.py --checkpoint results_vit2_cache\fold_1\best_model.pt --embedding-cache results_vit2_cache\vit_embeddings.pt --image-path "path\to\2024_05_10_10X.jpeg" --image-dir "path\to\station_day_images" --planting-date 2023-11-15 --device cuda
```

If the target filename does not contain a date, provide it manually:

```powershell
python ViT2\infer_cached_window.py --checkpoint results_vit2_cache\fold_1\best_model.pt --embedding-cache results_vit2_cache\vit_embeddings.pt --image-path "path\to\target.jpeg" --image-dir "path\to\station_day_images" --target-date 2024-05-10 --planting-date 2023-11-15 --device cuda
```
...
```powershell
python ViT2\infer_cached_window.py --checkpoint results_vit2_cache\fold_1\best_model.pt --embedding-cache results_vit2_cache\vit_embeddings.pt --image-path "path\to\2024_05_10_10X.jpeg" --image-dir "path\to\station_day_images" --planting-date 2023-11-15 --station-code 02.02 --weather-cache results_vit2_weather\meteostat_weather_cache.csv --device cuda
```
...
```powershell
python ViT2\infer_single_image.py --checkpoint results_vit2_full\fold_1\best_model.pt --image-path "path\to\10X.jpeg" --planting-date 2023-11-15 --device cuda
```

For a `10X` image:

```powershell
python ViT2\infer_single_image.py --checkpoint results_vit2_full\fold_1\best_model.pt --image-path "path\to\10X.jpeg" --stream micro --planting-date 2023-11-15 --device cuda
```

Best quick test, if matching same-day images exist:

```powershell
python ViT2\infer_single_image.py --checkpoint results_vit2_full\fold_1\best_model.pt --macro-path "path\to\1X.jpeg" --micro-path "path\to\10X.jpeg" --stream both --target-date 2024-05-10 --planting-date 2023-11-15 --device cuda
```
