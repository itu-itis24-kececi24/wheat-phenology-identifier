...
# Edit these values to change the fold split for ViT2 experiments.
...
def main():
    ...
    parser.add_argument("--exclude-offseason", action="store_true", help="Drop OffSeason rows and train only on PS1 through PS8.")
    ...
    if args.exclude_offseason:
        ...
        if daily_df.empty:
            raise RuntimeError("No PS1-PS8 rows remain after --exclude-offseason filtering.")
    ...
    log(
        "ViT2 fold station counts from file constants: "
        f"train={n_train_groups} val={n_val_groups} test={n_test_groups}"
    )
    log(
        "ViT2 fold station diversity requirements: "
