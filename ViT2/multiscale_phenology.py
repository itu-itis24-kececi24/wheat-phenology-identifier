...
def add_weather_metadata(
    ...
    for _, group_idx in out.groupby("group_id").groups.items():
        ...
        out.loc[group.index, "weather_gdd_cum_norm"] = gdd.cumsum().to_numpy() / WEATHER_CUM_GDD_SCALE
...
class SMOTEEmbeddingWindowDataset(Dataset):
    ...
    Synthetic examples are interpolations between nearest training windows with
    the same hard stage label.  This deliberately operates on frozen embedding
    tensors rather than image pixels, which is the meaningful place to apply
    SMOTE in this pipeline.  It must only wrap a training dataset.
    """
    ...
    def _build_synthetic_specs(self) -> Dict[str, object]:
        ...
        # Augmented cache views are alternate renderings of the same biological
        # window, so neighbour search uses each real window once. Their copies
        # still count toward the effective class balance in the DataLoader.
        ...
        if self.max_synthetic_samples > 0:
            ...
            if requested_total > self.max_synthetic_samples:
                ...
                for label in sorted(requested_by_class, key=lambda item: (-class_counts[item], item)):
                    if remaining <= 0:
                        break
                    if requested_by_class[label] < max(0, int(math.ceil(largest_class_count * self.target_ratio)) - class_counts[label]):
