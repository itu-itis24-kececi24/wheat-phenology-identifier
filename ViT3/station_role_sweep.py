...
def group_station_labels(daily_df: pd.DataFrame) -> Dict[str, str]:
    mapping = {}
    for group_id, group in daily_df.groupby("group_id"):
...
def summarize_station_roles(
    ...
    station_labels = group_station_labels(daily_df)
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Precompute ViT embeddings once, train every 12-train/3-val/no-test station combination, "
    ...
    parser.add_argument("--out-dir", default="results_vit3_station_role_sweep")
    ...
    groups = sorted(daily_df["group_id"].dropna().unique().tolist())
    ...
    summary = summarize_station_roles(val_metrics_path, daily_df, out_dir)
