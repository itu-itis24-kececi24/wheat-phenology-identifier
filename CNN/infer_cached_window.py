...
def find_images_by_date(image_dir: str, stream: str = "micro") -> Dict[pd.Timestamp, str]:
    ...
    for dirpath, _, filenames in os.walk(image_dir):
        for filename in filenames:
            ...
            date = date.normalize()
            if date in by_date:
                duplicate_counts[date] = duplicate_counts.get(date, 1) + 1
...
def load_tiling_config(checkpoint: Dict, embedding_cache: Optional[Dict], args) -> Dict:
    ...
    tiling.setdefault("vit_image_size", tiling.get("image_size", args.vit_image_size))
    return tiling
...
def build_temporal_features(
    ...
    return torch.stack(
        [
            _temporal_features_for_date(start_date + pd.Timedelta(days=offset), planting)
...
def main():
    ...
    parser.add_argument("--embedding-cache", default=None, help="Optional cnn_embeddings.pt used to read tiling/backbone settings.")
    ...
    parser.add_argument("--pretrained", action="store_true", help="Use pretrained CNN if no embedding cache is supplied.")
    ...
    encoder = CNNBackboneFeatureExtractor(backbone, pretrained=pretrained).to(device)
    encoder.eval()
