...
# Edit these values to change the fold split for CNN experiments.
...
# When enabled, validation/test groups must include at least two station-code
# prefixes when that split contains more than one group, e.g. 02.02 + 06.01.
...
def save_confusion_matrix(prediction_rows, out_dir: str, name: str):
    if not prediction_rows:
        log(f"No prediction rows available; skipping confusion matrix for {name}")
        return

    os.makedirs(out_dir, exist_ok=True)
    matrix = pd.DataFrame(0, index=BASE_CLASSES, columns=BASE_CLASSES, dtype=int)
    ...
    ax.set_yticks(range(len(BASE_CLASSES)))
    ax.set_xticklabels(BASE_CLASSES, rotation=45, ha="right")
    ax.set_yticklabels(BASE_CLASSES)
    for i, true_label in enumerate(BASE_CLASSES):
        for j, pred_label in enumerate(BASE_CLASSES):
...
def build_model(args, cfg, embedding_cache, device):
    temporal_feature_dim = TEMPORAL_FEATURE_DIM if args.use_days_since_planting else 0
                ...
                prediction_rows.append(
                    {
                        ...
                        "true_label": BASE_CLASSES[int(true_idx)],
                        "pred_label": BASE_CLASSES[int(pred_idx)],
                    }
                )
...
def main():
    ...
    parser.add_argument("--image-backbone", default=DEFAULT_IMAGE_BACKBONE, help="Torchvision image backbone for direct image training, e.g. efficientnet_v2_s, efficientnet_v2_m, convnext_tiny.")
    ...
    log(
        "CNN fold station counts from file constants: "
        f"train={n_train_groups} val={n_val_groups} test={n_test_groups}"
    )
    log(
        "CNN fold station diversity requirements: "
    ...
    if args.embedding_cache:
        ...
        log("No embedding cache supplied; training will load images and run the CNN image backbone each epoch")
    ...
    for fold_id, (train_idx, val_idx, test_idx) in enumerate(folds, 1):
        ...
        model = build_model(args, cfg, embedding_cache, device)
        if is_resume_fold:
        ...
        save_confusion_matrix(fold_test_predictions, fold_dir, "test")
    ...
    save_confusion_matrix(all_test_predictions, args.out_dir, "all_test")
