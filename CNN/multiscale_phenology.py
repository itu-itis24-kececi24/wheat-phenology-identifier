...
DEFAULT_IMAGE_BACKBONE = "efficientnet_v2_s"
...
def _date_window_scores(
    ...
    if "OffSeason" in class_to_idx:
        pre_dist = _distance_to_interval(date, first_date, boundaries[0] - pd.Timedelta(days=1))
...
def generate_group_train_val_test_folds(
    ...
    def station_family(value: object) -> str:
        try:
    ...
    if station_col in meta_df.columns:
        for group, group_df in meta_df.groupby(group_col):
            ...
            group_to_family[group] = station_family(values[0]) if len(values) else str(group)
    else:
...
class MultiScaleWindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        for step in range(self.config.window_days):
            ...
            temporal_features.append(_temporal_features_for_date(current_date, planting_date))
...
def _temporal_features_for_date(date: pd.Timestamp, planting_date: Optional[pd.Timestamp]) -> torch.Tensor:
    if planting_date is None or pd.isna(planting_date):
        return torch.zeros(TEMPORAL_FEATURE_DIM, dtype=torch.float32)
    days_since_planting = (pd.Timestamp(date).normalize() - pd.Timestamp(planting_date).normalize()).days
    return torch.tensor([days_since_planting / DAYS_SINCE_PLANTING_SCALE], dtype=torch.float32)
...
class MultiScaleEmbeddingWindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        for step in range(self.config.window_days):
            ...
            temporal_features.append(_temporal_features_for_date(current_date, planting_date))
            macro_features.append(self._load_embedding(macro_input_path, "macro"))
            micro_features.append(self._load_embedding(micro_input_path, "micro"))
...
class CNNFeatureEncoder(nn.Module):
    def __init__(self, backbone: str = DEFAULT_IMAGE_BACKBONE, pretrained: bool = True, out_dim: int = 512):
        super().__init__()
        if tvm is None:
            raise ImportError("torchvision is required for CNNFeatureEncoder")
        weights = _torchvision_model_weights(backbone, pretrained)
        base = getattr(tvm, backbone)(weights=weights)
        in_dim = _replace_classifier_with_identity(base)
        self.backbone = base
...
class CNNBackboneFeatureExtractor(nn.Module):
    def __init__(self, backbone: str = DEFAULT_IMAGE_BACKBONE, pretrained: bool = True):
        super().__init__()
        if tvm is None:
            raise ImportError("torchvision is required for CNNBackboneFeatureExtractor")
        weights = _torchvision_model_weights(backbone, pretrained)
        base = getattr(tvm, backbone)(weights=weights)
        self.out_dim = _replace_classifier_with_identity(base)
        self.backbone = base
...
# Backward-compatible aliases for older scripts/checkpoints that imported the ViT names.
ViTFeatureEncoder = CNNFeatureEncoder
ViTBackboneFeatureExtractor = CNNBackboneFeatureExtractor
...
def _replace_classifier_with_identity(base: nn.Module) -> int:
    if hasattr(base, "classifier"):
        ...
        if isinstance(classifier, nn.Linear):
            ...
            if not linear_layers:
                raise ValueError("Could not find a Linear classifier layer in torchvision model")
            in_dim = linear_layers[-1].in_features
    ...
    if hasattr(base, "heads"):
        head = getattr(base.heads, "head", None)
    ...
    raise ValueError(
        "Unsupported torchvision backbone classifier layout. "
        "Expected .classifier, .fc, or .heads.head."
    )


def _torchvision_model_weights(backbone: str, pretrained: bool):
    if not pretrained:
        return None

    if hasattr(tvm, "get_model_weights"):
        try:
            weights_enum = tvm.get_model_weights(backbone)
            return weights_enum.DEFAULT
        except (AttributeError, ValueError):
            pass

    weights_enum_overrides = {
    ...
    weights_enum_name = weights_enum_overrides.get(backbone, f"{backbone}_Weights")
...
def _torchvision_vit_weights(backbone: str, pretrained: bool):
    return _torchvision_model_weights(backbone, pretrained)
...
class TemporalAggregationMixin:
    ...
    def _init_temporal_features(self, embed_dim: int, temporal_feature_dim: int, dropout: float):
        ...
        if self.temporal_feature_dim > 0:
            self.temporal_feature_fusion = nn.Sequential(
                nn.LayerNorm(embed_dim + self.temporal_feature_dim),
                nn.Linear(embed_dim + self.temporal_feature_dim, embed_dim),
                nn.GELU(),
                nn.Dropout(dropout),
            )
        else:
...
class MultiScaleTemporalTransformer(nn.Module, TemporalAggregationMixin):
    def __init__(
        ...
        self._init_temporal_features(embed_dim, temporal_feature_dim, dropout)
        self.macro_encoder = CNNFeatureEncoder(image_backbone, pretrained=pretrained, out_dim=embed_dim)
        self.micro_encoder = CNNFeatureEncoder(image_backbone, pretrained=pretrained, out_dim=embed_dim)
...
class SingleStreamTemporalTransformer(nn.Module, TemporalAggregationMixin):
    def __init__(
        ...
        self._init_temporal_aggregation(embed_dim, temporal_aggregation)
        self._init_temporal_features(embed_dim, temporal_feature_dim, dropout)
        self.encoder = CNNFeatureEncoder(image_backbone, pretrained=pretrained, out_dim=embed_dim)
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=embed_dim,
            nhead=temporal_heads,
            dim_feedforward=embed_dim * 4,
...
class OrdinalRegressionLoss(nn.Module):
    ...
    The model still predicts one logit per class. Instead of comparing only the
    class probabilities, this compares cumulative distributions, so predicting a
    neighboring stage is penalized less than predicting a far-away stage.
    """
