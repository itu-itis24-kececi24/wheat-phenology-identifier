...
- `framework.py`: deprecated legacy notebook helper. The active CNN pipeline does not import it.
- `multiscale_phenology.py`: metadata builder, Excel/date normalization, station-folder matching, group folds, datasets, EfficientNet/CNN feature encoders, temporal Transformer models, and ordinal/soft-label losses.
...
The active CNN pipeline is self-contained in `multiscale_phenology.py` for metadata creation, label mapping, station lookup, image pairing, `OffSeason` windows, soft labels, temporal windows, and model definitions.

## Architecture Overview

The project is a temporal classifier. By default in `CNN`, it predicts the last day in a causal rolling window.
...
In full-image training, every image is passed through an EfficientNetV2/CNN backbone during training:

```text
1X image sequence
  -> macro EfficientNet/CNN encoder
  -> macro daily feature vectors

10X image sequence
  -> micro EfficientNet/CNN encoder
...
The temporal Transformer attends across the previous days in the window, so it can use recent growth history without relying on future photos.
...
The cached workflow freezes the image CNN backbone. It first computes image features once:

```text
all 1X/10X JPEGs -> frozen EfficientNet/CNN -> saved feature vectors
```
...
The default image backbone is `efficientnet_v2_s`. You can also try other torchvision CNN backbones, for example:

```powershell
--image-backbone efficientnet_v2_m
...
By default, each day in the temporal window also receives one metadata feature:

```text
days_since_planting / 365
```

The planting date comes from the label table's `1-Ekim` column. For the 30 pre-planting days this value is negative, then becomes `0` on planting day and increases through the season. Disable this ablation with:
...
```text
OffSeason, PS1, PS2, PS3, PS4, PS5, PS6, PS7, PS8
```
...
- labels Excel `PS1 - Emergence` through `PS7 - Maturity`,
- labels `8 - Olgunlaşma` to `9 - Hasat` as `PS8 - PreHarvest`,
...
This CNN copy is meant to be run directly with the scripts below. The default backbone is `efficientnet_v2_s`.
...
Option 1: run the helper script from `CNN`:
...
```python
from CNN.multiscale_phenology import build_multiscale_daily_dataframe
...
```powershell
python CNN\run_multiscale_training.py --excel-path labeling.xlsx ...
python CNN\run_multiscale_training.py --label-path labeling_iso_dates.csv ...
```
...
A healthy result should include all `PS1` to `PS8`, plus `OffSeason`. The `macro_path` and `micro_path` availability should be high, ideally close to `1.0`.
...
```powershell
python CNN\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_smoke_cnn --epochs 1 --folds 1 --window-days 3 --window-mode causal --stream micro --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 1 --accumulation-steps 1 --num-workers 0 --camera AUTO --pretrained --device cuda --log-interval 1
```
...
For `CNN`, validation/test station counts are edited in [run_multiscale_training.py](run_multiscale_training.py), not passed as command-line arguments:
...
For practical experiments, freeze the image CNN backbone and precompute image embeddings once.

For `10X` close-up images, use tiled embeddings so the full `2288x1712` image is not squeezed into one `224x224` view. The script crops native-resolution tiles first and runs EfficientNet/CNN on each tile. With `--tile-pooling attention`, it stores all tile features and the training model learns which tiles matter.
...
```powershell
python CNN\precompute_multiscale_embeddings.py --excel-path labeling.xlsx --data-path data --out-dir results_cnn_cache --camera AUTO --stream micro --tile-streams micro --tile-pooling attention --tile-size 224 --tile-stride 224 --max-tiles 0 --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 64 --num-workers 4 --embedding-dtype float16 --pretrained --device cuda
```

Step 2: train from cached vectors:

```powershell
python CNN\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_cnn_cache --embedding-cache results_cnn_cache\cnn_embeddings.pt --epochs 5 --folds 1 --window-days 31 --window-mode causal --stream micro --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 32 --accumulation-steps 1 --num-workers 0 --camera AUTO --device cuda --log-interval 10
```

This is much faster because training no longer decodes JPEGs or runs EfficientNet/CNN for every frame during every epoch.
...
- `--vit-image-size 224`: CNN input size after tile crop. The argument name is kept for compatibility.
...
Rebuild `cnn_embeddings.pt` whenever you change:
...
```powershell
python CNN\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_cnn_full --epochs 20 --folds 5 --window-days 31 --window-mode causal --stream micro --preplant-days 30 --postharvest-days 30 --date-tolerance-days 7 --batch-size 2 --accumulation-steps 16 --num-workers 4 --camera AUTO --image-backbone efficientnet_v2_s --pretrained --device cuda --log-interval 25
```

CUDA automatic mixed precision is enabled by default to reduce VRAM use and speed up CNN training. Disable it only for debugging:
...
```powershell
python CNN\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_cnn_cache --embedding-cache results_cnn_cache\cnn_embeddings.pt --epochs 5 --folds 1 --window-days 31 --window-mode causal --stream micro --device cuda --compile-model --compile-mode reduce-overhead
```
...
Epoch curves are available in TensorBoard. Confusion matrices are saved after final held-out test evaluation.
...
```powershell
python CNN\run_multiscale_training.py --excel-path labeling.xlsx --data-path data --out-dir results_cnn_cache --embedding-cache results_cnn_cache\cnn_embeddings.pt --resume-checkpoint results_cnn_cache\last_checkpoint.pt --epochs 20 --device cuda
```
...
```powershell
--resume-checkpoint results_cnn_cache\fold_3\last_checkpoint.pt
```
...
```powershell
tensorboard --logdir results_cnn_cache\tensorboard
```
...
```powershell
python CNN\infer_cached_window.py --checkpoint results_cnn_cache\fold_1\best_model.pt --embedding-cache results_cnn_cache\cnn_embeddings.pt --image-path "path\to\target_10X.jpeg" --planting-date 2023-11-15 --device cuda
```
...
```powershell
python CNN\infer_cached_window.py --checkpoint results_cnn_cache\fold_1\best_model.pt --embedding-cache results_cnn_cache\cnn_embeddings.pt --image-path "path\to\2024_05_10_10X.jpeg" --image-dir "path\to\station_day_images" --planting-date 2023-11-15 --device cuda
```

If the target filename does not contain a date, provide it manually:

```powershell
python CNN\infer_cached_window.py --checkpoint results_cnn_cache\fold_1\best_model.pt --embedding-cache results_cnn_cache\cnn_embeddings.pt --image-path "path\to\target.jpeg" --image-dir "path\to\station_day_images" --target-date 2024-05-10 --planting-date 2023-11-15 --device cuda
```
...
```powershell
python CNN\infer_single_image.py --checkpoint results_cnn_full\fold_1\best_model.pt --image-path "path\to\10X.jpeg" --planting-date 2023-11-15 --device cuda
```

For a `10X` image:

```powershell
python CNN\infer_single_image.py --checkpoint results_cnn_full\fold_1\best_model.pt --image-path "path\to\10X.jpeg" --stream micro --planting-date 2023-11-15 --device cuda
```

Best quick test, if matching same-day images exist:

```powershell
python CNN\infer_single_image.py --checkpoint results_cnn_full\fold_1\best_model.pt --macro-path "path\to\1X.jpeg" --micro-path "path\to\10X.jpeg" --stream both --target-date 2024-05-10 --planting-date 2023-11-15 --device cuda
```
