...
def main():
    ...
    if stream == "both" and args.image_path:
    ...
    macro_valid = torch.full((window_days,), macro_image is not None, dtype=torch.bool)
    micro_valid = torch.full((window_days,), micro_image is not None, dtype=torch.bool)
    mask = macro_valid | micro_valid
    ...
    with torch.no_grad():
        ...
        if stream == "both":
            logits = model(
                *model_args,
                macro_valid=macro_valid.unsqueeze(0).to(device),
                micro_valid=micro_valid.unsqueeze(0).to(device),
            )
        else:
    ...
    result = {
        ...
        "macro_available": bool(macro_image is not None),
        "micro_available": bool(micro_image is not None),
        "temporal_model": temporal_model,
