...
def encode_tiled_paths(
    ...
    for idx, item in enumerate(loader, 1):
        ...
        for start in range(0, tiles.shape[0], batch_size):
            ...
            with autocast_context(device, amp_dtype):
                features.append(model(batch).detach().to("cpu", dtype=output_dtype))
...
def main():
    ...
    parser.add_argument("--out-dir", default="results_dinov2_bbch_gated_cache")
    ...
    parser.add_argument("--stream", choices=["micro", "macro", "both"], default="both", help="Which image stream to cache. both is required for gated fusion.")
    ...
    parser.add_argument("--amp", choices=["auto", "float16", "bfloat16", "none"], default="auto", help="Mixed precision used for frozen DINOv2 extraction.")
    parser.add_argument("--tile-streams", choices=["none", "micro", "macro", "both"], default="both", help="Which streams use native-resolution tiling before DINOv2 encoding.")
