...
class SMOTEEmbeddingWindowDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        for key in ("mask", "macro_tile_mask", "micro_tile_mask", "macro_valid", "micro_valid"):
            if key in source:
...
class GatedMultiviewFusion(nn.Module):
    """Fuse daily macro/micro features while respecting missing cameras."""
    ...
    def __init__(
        ...
        self.post = nn.Sequential(nn.LayerNorm(embed_dim), nn.GELU(), nn.Dropout(dropout))
        # Begin as an unbiased 50/50 mixture when both cameras are present.
    ...
    def forward(
        ...
        if self.temporal_feature_dim > 0:
            ...
            expected_shape = (*macro.shape[:2], self.temporal_feature_dim)
            if tuple(temporal_features.shape) != expected_shape:
                raise ValueError(
                    f"temporal_features must have shape {expected_shape}, "
...
class MultiScaleEmbeddingTemporalTransformer(nn.Module, TemporalAggregationMixin):
    def __init__(
        ...
        self._init_temporal_aggregation(embed_dim, temporal_aggregation)
        self._init_temporal_features(embed_dim, temporal_feature_dim, dropout, temporal_feature_hidden_dim)
        self.macro_tile_pool = TileAttentionPooler(feature_dim)
        self.micro_tile_pool = TileAttentionPooler(feature_dim)
        self.fusion = GatedMultiviewFusion(
    ...
    def forward(
        ...
        macro = self.macro_tile_pool(macro, macro_tile_mask)
        micro = self.micro_tile_pool(micro, micro_tile_mask)
