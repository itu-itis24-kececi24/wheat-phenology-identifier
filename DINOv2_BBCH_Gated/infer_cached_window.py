...
def infer_date(args) -> pd.Timestamp:
    ...
    for path in (args.image_path, args.micro_path, args.macro_path):
        if not path:
            continue
        date = parse_image_date(path)
        if date is not None:
...
def build_temporal_features(
    ...
    if needs_weather and weather_cache and station_code:
        ...
        print(
            "Warning: checkpoint expects weather metadata but --weather-cache/--station-code was not supplied; "
            "weather values are zero and missingness indicators are one.",
            flush=True,
        )
...
def temporal_feature_columns_from_checkpoint(checkpoint: Dict) -> List[str]:
    ...
    expected_dim = int(checkpoint.get("temporal_feature_dim", len(columns)))
    return columns[:expected_dim]
...
def build_model(checkpoint: Dict, feature_dim: int, classes: List[str], stream: str, device: torch.device):
    ...
    temporal_norm_first = bool(checkpoint.get("temporal_norm_first", False))
    temporal_ffn_multiplier = float(checkpoint.get("temporal_ffn_multiplier", 4.0))
    if stream == "both":
        model = MultiScaleEmbeddingTemporalTransformer(
            ...
            gate_hidden_dim=int(checkpoint.get("gate_hidden_dim", 128)),
            modality_dropout=float(checkpoint.get("modality_dropout", 0.0)),
            temporal_norm_first=temporal_norm_first,
            temporal_ffn_multiplier=temporal_ffn_multiplier,
        ).to(device)
    else:
...
def main():
    ...
    parser.add_argument("--image-path", default=None, help="Target-day image for a single-stream checkpoint.")
    parser.add_argument("--image-dir", default=None, help="Previous-day folder for a single-stream checkpoint.")
    parser.add_argument("--macro-path", default=None, help="Target-day 1X/canopy image for a gated checkpoint.")
    parser.add_argument("--micro-path", default=None, help="Target-day 10X/close-up image for a gated checkpoint.")
    parser.add_argument("--macro-dir", default=None, help="Folder containing previous 1X/canopy images.")
    parser.add_argument("--micro-dir", default=None, help="Folder containing previous 10X/close-up images.")
    parser.add_argument("--target-date", default=None, help="Target date YYYY-MM-DD. Defaults to a supplied image filename.")
    parser.add_argument("--planting-date", default=None, help="1-Sowing date in YYYY-MM-DD format. Required for non-zero days-since-planting metadata.")
    ...
    embedding_cache = torch.load(args.embedding_cache, map_location="cpu", weights_only=False) if args.embedding_cache else None
    classes = checkpoint.get("classes", BASE_CLASSES)
    stream = args.stream or checkpoint.get("stream", "micro")
    if stream == "both":
        if args.image_path:
            raise ValueError("For a gated checkpoint, use --macro-path and/or --micro-path instead of --image-path.")
        if not args.macro_path and not args.micro_path:
            raise ValueError("A gated checkpoint requires --macro-path and/or --micro-path.")
    elif not args.image_path:
        candidate = args.micro_path if stream == "micro" else args.macro_path
        if candidate:
            args.image_path = candidate
        else:
            raise ValueError(f"A {stream} checkpoint requires --image-path.")
    ...
    if station_code is None:
        for path in (args.image_path, args.micro_path, args.macro_path):
            station_code = parse_station_code(path)
            if station_code:
                break
    ...
    if stream == "both":
        macro_paths = build_window_paths(args.macro_path, args.macro_dir, target_date, window_days, "macro")
        micro_paths = build_window_paths(args.micro_path, args.micro_dir, target_date, window_days, "micro")
    else:
        active_paths = build_window_paths(args.image_path, args.image_dir, target_date, window_days, stream)
        macro_paths = active_paths if stream == "macro" else [None] * window_days
        micro_paths = active_paths if stream == "micro" else [None] * window_days

    if args.debug_window:
        start_date = target_date - pd.Timedelta(days=window_days - 1)
        print("Resolved inference window:", flush=True)
        for offset, (macro_path, micro_path) in enumerate(zip(macro_paths, micro_paths)):
            date = start_date + pd.Timedelta(days=offset)
            print(
                f"  {date.date()} macro={'FOUND' if macro_path else 'MISSING'} {macro_path or ''} "
                f"micro={'FOUND' if micro_path else 'MISSING'} {micro_path or ''}",
                flush=True,
            )

    encoded_by_path = {}
    all_paths = {path_key(path) for path in macro_paths + micro_paths if path is not None}
    ...
    if not encoded_by_path:
        raise RuntimeError("No supplied inference image could be encoded.")
    reference_feature = next(iter(encoded_by_path.values()))
    macro_target = encoded_by_path.get(path_key(args.macro_path)) if args.macro_path else None
    micro_target = encoded_by_path.get(path_key(args.micro_path)) if args.micro_path else None
    if stream == "macro":
        macro_target = encoded_by_path.get(path_key(args.image_path))
    elif stream == "micro":
        micro_target = encoded_by_path.get(path_key(args.image_path))

    def feature_sequence(paths, target_feature):
        features, valid = [], []
        for path in paths:
            if path is not None:
                features.append(encoded_by_path[path_key(path)])
                valid.append(True)
            elif args.repeat_missing and target_feature is not None:
                features.append(target_feature)
                valid.append(True)
            else:
                features.append(torch.zeros_like(reference_feature, dtype=torch.float32))
                valid.append(False)
        return features, valid

    macro_features, macro_valid = feature_sequence(macro_paths, macro_target)
    micro_features, micro_valid = feature_sequence(micro_paths, micro_target)
    mask = [macro_ok or micro_ok for macro_ok, micro_ok in zip(macro_valid, micro_valid)]
    if not mask[-1]:
        raise RuntimeError("The target day has neither a usable macro nor micro image.")
    ...
    if tiling.get("tile_pooling") == "attention":
        macro, macro_tile_mask = pad_tile_features(macro_features, feature_dim)
        micro, micro_tile_mask = pad_tile_features(micro_features, feature_dim)
        macro_valid_tensor = torch.tensor(macro_valid, dtype=torch.bool)
        micro_valid_tensor = torch.tensor(micro_valid, dtype=torch.bool)
        macro_tile_mask &= macro_valid_tensor.unsqueeze(-1)
        micro_tile_mask &= micro_valid_tensor.unsqueeze(-1)
        macro = macro.unsqueeze(0).to(device)
        micro = micro.unsqueeze(0).to(device)
        macro_tile_mask = macro_tile_mask.unsqueeze(0).to(device)
        micro_tile_mask = micro_tile_mask.unsqueeze(0).to(device)
        with torch.no_grad():
            if stream == "both":
                logits = model(
                    macro,
                    ...
                    micro_tile_mask,
                    temporal_features,
                    macro_valid_tensor.unsqueeze(0).to(device),
                    micro_valid_tensor.unsqueeze(0).to(device),
                )
            else:
                logits = model(macro, micro, mask_tensor, macro_tile_mask, micro_tile_mask, temporal_features)
    else:
        macro = torch.stack([feat.float() if feat.ndim == 1 else feat.float().mean(dim=0) for feat in macro_features])
        micro = torch.stack([feat.float() if feat.ndim == 1 else feat.float().mean(dim=0) for feat in micro_features])
        macro = macro.unsqueeze(0).to(device)
        micro = micro.unsqueeze(0).to(device)
        macro_valid_tensor = torch.tensor(macro_valid, dtype=torch.bool).unsqueeze(0).to(device)
        micro_valid_tensor = torch.tensor(micro_valid, dtype=torch.bool).unsqueeze(0).to(device)
        with torch.no_grad():
            if stream == "both":
                logits = model(
                    macro,
                    ...
                    macro_valid=macro_valid_tensor,
                    micro_valid=micro_valid_tensor,
                )
            else:
    ...
    result = {
        ...
        "macro_path": os.path.abspath(args.macro_path) if args.macro_path else None,
        "micro_path": os.path.abspath(args.micro_path) if args.micro_path else None,
        "macro_dir": os.path.abspath(args.macro_dir) if args.macro_dir else None,
        "micro_dir": os.path.abspath(args.micro_dir) if args.micro_dir else None,
        "target_date": str(target_date.date()),
        "window_days": window_days,
        "available_window_images": int(sum(mask)),
        "missing_window_images": int(window_days - sum(mask)),
        "available_macro_images": int(sum(macro_valid)),
        "available_micro_images": int(sum(micro_valid)),
        "stream": stream,
        "temporal_model": checkpoint.get("temporal_model", "transformer"),
        "tile_pooling": tiling.get("tile_pooling", "attention"),
        ...
        "used_dates": [
            {
                ...
                "macro_path": os.path.abspath(macro_path) if macro_path else None,
                "micro_path": os.path.abspath(micro_path) if micro_path else None,
                "macro_available": bool(macro_valid[i]),
                "micro_available": bool(micro_valid[i]),
            }
            for i, (macro_path, micro_path) in enumerate(zip(macro_paths, micro_paths))
        ],
    }
